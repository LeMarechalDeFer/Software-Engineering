...
struct List trouver_element(List list, int find_index);

void supprimer_element(int key);
...
void insert(int key, int value){
	...
	int index = hashcode(key);  /* Extracting Linked List at a given index */
	
	List list = (List) array[index].head; /* Creating an item to insert in the Hash Table */
	
	List item = (List) malloc(sizeof(Element));
	...
	if (list == NULL) {
		printf("Inserting %d(key) and %d(value) \n", key, value); /* Absence of Linked List at a given Index of Hash Table */
		array[index].head = item;
		array[index].tail = item;
		size++;
	}
        else {
		int find_index = find(list, key); /* A Linked List is present at given index of Hash Table */
		if (find_index == -1){
			/*
			...
			*/
			List element = get_element(list, find_index);
			element->value = value;
