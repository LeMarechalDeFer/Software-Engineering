...
CREATE TABLE Artiste ( 
   Nom VARCHAR2(20) CONSTRAINT pk_artiste PRIMARY KEY, 
   Prenom VARCHAR2(15), 
   Annee_naissance NUMBER(4, 0) 
);

CREATE TABLE Cinema ( 
   Nom_cinema VARCHAR2(10) CONSTRAINT pk_cinema PRIMARY KEY, 
   Arrondissement NUMBER(2, 0), 
   Adresse VARCHAR2(30) 
);

CREATE TABLE Film ( 
   ID_film NUMBER(10, 0) CONSTRAINT pk_film PRIMARY KEY, 
   Titre VARCHAR2(30), 
   Annee NUMBER(4, 0), 
...
CREATE TABLE Salle ( 
   No_salle NUMBER(2, 0) CONSTRAINT pk_salle PRIMARY KEY,   
   Nom_cinema VARCHAR2(10) CONSTRAINT fk_cinema REFERENCES Cinema(Nom_cinema),        
   Climatise CHAR(1), 
   Capacite NUMBER(4, 0)    
);

CREATE TABLE Role ( 
   Nom_Role VARCHAR2(20) NOT NULL, 
   ID_film NUMBER(10, 0) CONSTRAINT fk_film REFERENCES Film(ID_film),  
   Nom_acteur VARCHAR2(20) CONSTRAINT fk_acteur REFERENCES Artiste(Nom),    
   CONSTRAINT pk_role PRIMARY KEY (Nom_acteur, ID_film) 
);

CREATE TABLE Seance ( 
   No_seance NUMBER(2, 0) CONSTRAINT pk_seance PRIMARY KEY, 
   Nom_cinema VARCHAR2(10) CONSTRAINT fk2_cinema REFERENCES Cinema(Nom_cinema),  
   No_salle NUMBER(2, 0) CONSTRAINT fk_salle REFERENCES Salle(No_salle),  
   ID_film NUMBER(10, 0) CONSTRAINT fk2_film REFERENCES Film(ID_film),    
